<?php
	 ...
	 * @package property
	 * @subpackage budget
	 * @version $Id: class.sobudget.inc.php 17164 2017-10-18 13:36:13Z sigurdne $
	 */
...
//			_debug_array($data);
			...
			if ($direction == 'income')
			{
				$filtermethod_direction = "AND fm_b_account.id {$this->like} '3%'";
			}
			else if($direction == 'expenses')
			{
				$filtermethod_direction = "AND fm_b_account.id NOT {$this->like} '3%'";
			}
			...
			$sql = "SELECT DISTINCT fm_workorder.id AS id, fm_location1.mva,fm_workorder.project_id,"
				...
				. " {$filtermethod_order}{$filtermethod} {$querymethod} {$filtermethod_direction}"
...
//			_debug_array($sql);
			...
			$sql = "SELECT DISTINCT fm_workorder.id AS id, fm_location1.mva,fm_workorder.project_id,"
				...
				. " {$_join_district}"
				. " {$this->join} fm_part_of_town ON fm_location1.part_of_town_id = fm_part_of_town.id"
				. " {$filtermethod_payment}{$filtermethod} {$querymethod} {$filtermethod_direction}"
				. " ORDER BY fm_workorder.id ASC";
...
//			_debug_array($obligations);
			...
			$sql = "SELECT sum(budget) as budget, count(fm_tts_tickets.id) as hits, fm_b_account.{$b_account_field} as {$b_account_field}, district_id, fm_tts_tickets.ecodimb"
				...
				. " {$this->join} fm_b_account ON fm_tts_tickets.b_account_id = fm_b_account.id "
				. " {$this->join} fm_location1 ON fm_tts_tickets.loc1 = fm_location1.loc1 "
				. " {$this->join} fm_part_of_town ON fm_location1.part_of_town_id = fm_part_of_town.id {$filtermethod} {$querymethod} {$filtermethod_direction} GROUP BY fm_b_account.{$b_account_field},district_id,fm_tts_tickets.ecodimb";
			...
			if (is_array($accout_info))
			{
				...
				foreach ($accout_info as $b_account)
				{
					foreach ($district as $district_id)
					{
						...
						{
							if ((isset($actual_cost[$b_account][$district_id][$dimb]) && $actual_cost[$b_account][$district_id][$dimb]) || (isset($budget_cost[$b_account][$district_id][$dimb]) && $budget_cost[$b_account][$district_id][$dimb]) || (isset($obligations[$b_account][$district_id][$dimb]) && $obligations[$b_account][$district_id][$dimb]))
							{
								$result[] = array(
