<?php
	 ...
	 * @package property
	 * @subpackage entity
	 * @version $Id: class.soentity.inc.php 17418 2017-12-09 17:45:33Z sigurdne $
	 */
...
//				$xml->loadXML($xmldata);
			...
			foreach ($dataset as &$entry)
			{
				$entry['short_description'] = array
					(
					'value' => $this->get_short_description(array('location_id' => $location_id,
						'id' => $entry['id']['value'])),
					'datatype' => false,
					'attrib_id' => false,
				);
...
//					$description_value = $xml->getElementsByTagName($attribute['name'])->item(0)->nodeValue;
		...
		function get_cols( $category, $entity_id, $cat_id, $lookup, $location_id )
		{
			...
			if (!$this->uicols)
			{
				...
				$this->sql = $this->bocommon->generate_sql(array('entity_table' => $entity_table,
					'cols_return' => $cols_return, 'cols' => $cols,
					'uicols' => $uicols, 'joinmethod' => $joinmethod, 'paranthesis' => $paranthesis,
					'query' => $query, 'lookup' => $lookup, 'location_level' => $category['location_level']));
