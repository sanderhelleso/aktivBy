...
<!-- $Id: filter_vendor.xsl 14719 2016-02-10 19:45:46Z sigurdne $ -->
<xsl:template name="filter_vendor">
		...
		<form method="post" action="{$select_action}">
			<select name="vendor_id" onChange="this.form.submit();" onMouseout="window.status='';return true;">
				<xsl:attribute name="title">
					<xsl:value-of select="lang_vendor_statustext"/>
				</xsl:attribute>
				<option value="">
					<xsl:value-of select="lang_no_vendor"/>
				</option>
