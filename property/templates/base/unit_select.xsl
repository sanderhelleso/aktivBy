...
<!-- $Id: unit_select.xsl 14719 2016-02-10 19:45:46Z sigurdne $ -->
<xsl:template name="unit_select">
		<xsl:variable name="lang_unit_statustext">
			<xsl:value-of select="lang_unit_statustext"/>
		</xsl:variable>
		<xsl:variable name="select_name">
			<xsl:value-of select="select_unit"/>
		</xsl:variable>
		<select name="{$select_name}" class="forms" onMouseover="window.status='{$lang_unit_statustext}'; return true;" onMouseout="window.status='';return true;">
			<option value="">
				<xsl:value-of select="lang_no_unit"/>
			</option>
