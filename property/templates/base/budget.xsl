...
<!-- $Id: budget.xsl 14861 2016-03-25 12:37:53Z sigurdne $ -->
<xsl:template match="data">
		<xsl:choose>
			<xsl:when test="edit">
				<xsl:apply-templates select="edit"/>
			</xsl:when>
		<xsl:when test="edit_basis">
			<xsl:apply-templates select="edit_basis"/>
		</xsl:when>
			<xsl:when test="view">
...
<xsl:template match="table_header_budget_basis">
		<xsl:variable name="sort_district_id">
			<xsl:value-of select="sort_district_id"/>
		</xsl:variable>
		<xsl:variable name="sort_budget_cost">
			<xsl:value-of select="sort_budget_cost"/>
		</xsl:variable>
		<xsl:variable name="sort_b_account">
			<xsl:value-of select="sort_b_account"/>
		</xsl:variable>
		<xsl:variable name="sort_grouping">
			<xsl:value-of select="sort_grouping"/>
		</xsl:variable>
		<tr class="th">
...
<xsl:template match="edit_basis">
	...
	<form name="form" id="form" method="post" action="{$edit_url}" class="pure-form pure-form-aligned">
			...
			<div id="generic">
				...
				<div class="pure-control-group">
					<label>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
									<xsl:variable name="lang_year_statustext">
				...
				<div class="pure-control-group">
					<label>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
									<xsl:variable name="lang_revision_statustext">
				...
				<div class="pure-control-group">
					<label>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
									<xsl:variable name="lang_b_group_statustext">
				...
				<div class="pure-control-group">
					<label>
							...
							<xsl:variable name="lang_distribute_year_statustext">
								<xsl:value-of select="lang_distribute_year_statustext"/>
							</xsl:variable>
		...
		<div class="proplist-col">
			...
			<input type="button" class="pure-button pure-button-primary" name="values[cancel]" value="{$lang_cancel}" onClick="document.done_form.submit();" onMouseout="window.status='';return true;">
								<xsl:attribute name="onMouseover">
									<xsl:text>window.status='</xsl:text>
...
<xsl:template match="values_budget_obligations">
		<tr>
			...
			<td align="right">
				<xsl:variable name="link_obligation">
					<xsl:value-of select="link_obligation"/>
				</xsl:variable>
				<a href="{$link_obligation}" onMouseover="window.status='{obligation}';return true;" onMouseout="window.status='';return true;">
					<xsl:value-of select="obligation"/>
				</a>
			</td>
			<td align="right">
				<xsl:variable name="link_actual_cost">
					<xsl:value-of select="link_actual_cost"/>
				</xsl:variable>
				<a href="{$link_actual_cost}" onMouseover="window.status='{actual_cost}';return true;" onMouseout="window.status='';return true;">
...
<xsl:template name="filter_year">
		...
		<form method="post" action="{$select_action}">
			<select name="year" onChange="this.form.submit();" onMouseout="window.status='';return true;">
				...
				<option value="">
					<xsl:value-of select="lang_no_year"/>
				</option>
...
<xsl:template name="filter_revision">
		...
		<form method="post" action="{$select_action}">
			<select name="revision" onChange="this.form.submit();" onMouseout="window.status='';return true;">
				...
				<option value="">
					<xsl:value-of select="lang_no_revision"/>
				</option>
				<xsl:apply-templates select="revision_list"/>
			</select>
			<noscript>
