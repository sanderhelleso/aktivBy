<?php
  ...
  *  option) any later version.                                              *
  \**************************************************************************/
	...
	/* $Id: class.uiholiday.inc.php 13899 2015-09-15 09:23:48Z sigurdne $ */
...
// Locale
			$this->display_item($t,lang('Country'),'<input name="holiday[locale]" size="3" maxlength="2" value="'.(isset($holiday['locale'])?$holiday['locale']:'').'">');
...
// Occurence
			...
			for($i=0;$i<7;$i++)
			{
				$out .= '<option value="'.$i.'"'.(isset($holiday['dow']) && $holiday['dow']==$i?' selected':'').'>'.$dow[$i].'</option>'."\n";
			}
			...
			$this->display_item($t,lang('Observance Rule'),'<input type="checkbox" name="holiday[observance_rule]" value="True"'.(isset($holiday['observance_rule']) && $holiday['observance_rule']?' checked':'').'>'.
