 ...
 * @version $Id: class.egw_db.inc.php 16988 2017-08-19 15:32:15Z sigurdne $
 */
...
*/
	...
	function connect($Database = NULL, $Host = NULL, $Port = NULL, $User = NULL, $Password = NULL,$Type = NULL)
	{
		...
		if (!isset($this->adodb) || !$this->adodb)
		{
					...
					$this->Port != $GLOBALS['phpgw']->db->Port)))
			{
				...
				if (($Ok = $this->adodb->$connect($Host, $User, $Password)))
				{
					$this->ServerInfo = $this->adodb->ServerInfo();
					$this->set_capabilities($type,$this->ServerInfo['version']);
					if($Database)
					{
					   $Ok = $this->adodb->SelectDB($Database);
					}
				}
				if (!$Ok)
				{
				...
				if ($this->Type == 'mssql')
				{
					...
					$this->adodb->Execute('SET DATEFORMAT ymd');
		...
		if (!$this->adodb->_connectionID) $this->adodb->Connect();

		if ($new_connection && $GLOBALS['phpgw_info']['server']['sessions_type'] == 'php4-restore')
		{
	...
	*/
	function query($Query_String, $line = '', $file = '', $offset=0, $num_rows=-1,$inputarr=false,$fetchmode=egw_db::FETCH_BOTH)
	{
		...
		if ($this->adodb->fetchMode != $fetchmode)
		{
			$this->adodb->SetFetchMode($fetchmode);
		}
		if (!$num_rows)
		{
		...
		if ($num_rows > 0)
		{
			$this->Query_ID = $this->adodb->SelectLimit($Query_String,$num_rows,(int)$offset,$inputarr);
		}
		else
		{
			$this->Query_ID = $this->adodb->Execute($Query_String,$inputarr);
		}
		$this->Row = 0;
	...
	*/
	...
	{
		if ($this->adodb)		// only if we have a link, else infinite loop
		{
			$this->Error = $this->adodb->ErrorMsg();	// need to be BEFORE unlock,
			$this->Errno = $this->adodb->ErrorNo();	// else we get its error or none
	...
	*/
	function metadata($table='',$full=false)
	{
		...
		$columns = $this->adodb->MetaColumns($table);
		//$columns = $this->adodb->MetaColumnsSQL($table);
...
//				_debug_array($column);
	...
	*/
	function pkey_columns($tablename)
	{
		...
		return $this->adodb->MetaPrimaryKeys($tablename);
	}
	...
	function create_database($adminname = '', $adminpasswd = '', $charset='')
	{
		...
		switch ($this->Type)
		{
			...
			case 'mysql':
				...
				if ($charset && isset($this->adodb->charset2mysql[$charset]) && (float) $this->ServerInfo['version'] >= 4.1)
				{
					$create .= ' DEFAULT CHARACTER SET '.$this->adodb->charset2mysql[$charset].';';
				}
				$sqls[] = $create;
	...
	*/
	function quote($value,$type=False,$not_null=true,$length=null,$glue=',')
	{
		...
		{
			case 'blob':
				switch ($this->adodb->blobEncodeType)
				{
					case 'C':	// eg. postgres
						return "'" . $this->adodb->BlobEncode($value) . "'";
					case 'I':
						return $this->adodb->BlobEncode($value);
				}
				break;	// handled like strings
			case 'date':
				return $this->adodb->DBDate($value);
			case 'timestamp':
				return $this->adodb->DBTimeStamp($value);
		}
		if (is_array($value))
		{
	...
	*/
	...
	{
		if ($app === true && $table && isset($GLOBALS['phpgw_info']['apps']))
		{
	...
	*/
	function insert($table,$data,$where,$line,$file,$app=False,$use_prepared_statement=false,$table_def=False)
	{
		...
		{
			$this->adodb->Param(false);	// reset param-counter
			...
			{
				$this->prepared_sql[$sql] = $this->adodb->Prepare($sql);
			}
	...
	*/
	function update($table,$data,$where,$line,$file,$app=False,$use_prepared_statement=false,$table_def=False)
	{
		...
		if (count($data))
		{
			...
			if ($use_prepared_statement && $this->adodb->_bindInputArray)	// eg. MaxDB
			{
				...
				foreach($data as $col => $val)
				{
					...
					$params[] = $this->name_quote($col).'='.$this->adodb->Param($col);
				}
		...
		if (($ret || !count($data)) && count($blobs2update))
		{
			...
			{
				$ret = $this->adodb->UpdateBlob($table,$col,$val,$where,$table_def['fd'][$col]['type'] == 'blob' ? 'BLOB' : 'CLOB');
