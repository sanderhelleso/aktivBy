<?php
	...
	* option) any later version.                                                *
	\***************************************************************************/
	/* $Id: class.felamimail_bosignatures.inc.php 13903 2015-09-15 10:32:01Z sigurdne $ */

	require_once(PHPGW_INCLUDE_ROOT.'/felamimail/inc/class.felamimail_signatures.inc.php');
	...
	class felamimail_bosignatures
	{
		...
		function getListOfSignatures() {
			...
			if(count($signatures) == 0 && 
				...
				$newSignature->save();
				$GLOBALS['phpgw']->preferences->add('felamimail', 'email_sig_copied', true);
		...
		function getSignature($_signatureID, $_unparsed = false) 
		{
			if($_signatureID == -1) {
				...
				$signature->fm_signature	= ($_unparsed === true ? $this->profileData->ea_default_signature : $GLOBALS['phpgw']->preferences->parse_notify($this->profileData->ea_default_signature));
