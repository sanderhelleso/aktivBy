<?php
	...
	* @version $Id: class.datetime.inc.php 12060 2014-08-21 12:40:25Z sigurdne $
	*/
	...
	*/
	class phpgwapi_datetime
	{
		...
		*/
		 ...
		 * @return int
		 */
		public static function get_working_days($start_date, $end_date, $holidays = array())
		{
			...
			if(!$holidays)
			{
				$holidays = self::get_holidays(date('Y',$end_date));
			}
		...
		public static function get_holidays($year = 0)
		{
			...
			(
				...
				date('Y-m-d', mktime(0, 0, 0, 12, 26, ($year-1))),
				date('Y-m-d', mktime(0, 0, 0, 1, 1, $year)),
				date('Y-m-d', (mktime(0, 0, 0, 3, 21, $year) + ( $maundy_thursday * 86400 ))),//Maundy Thursday
				date('Y-m-d', (mktime(0, 0, 0, 3, 21, $year) + (( $maundy_thursday +1 ) * 86400))),//Good Friday
				date('Y-m-d', (mktime(0, 0, 0, 3, 21, $year) + (( $easter_day +1 ) * 86400))),//Easter Monday
				date('Y-m-d', mktime(0, 0, 0, 5, 1, $year)),
				date('Y-m-d', mktime(0, 0, 0, 5, 17, $year)),//FIXME: Then Norwegian Constitution Day...
				date('Y-m-d', (mktime(0, 0, 0, 3, 21, $year) + (( $easter_day + 39) * 86400))),//Ascension Day, 39 days after Easter Day
				date('Y-m-d', (mktime(0, 0, 0, 3, 21, $year) + (( $easter_day + 49) * 86400))),//Whit Sunday, 49 days after Easter Day
				date('Y-m-d', (mktime(0, 0, 0, 3, 21, $year) + (( $easter_day + 50) * 86400))),//Whit monday, 50 days after Easter Day
