<?php
	...
	* @package phpgwapi
	* @subpackage contacts
	* @version $Id: class.contacts_sql.inc.php 15508 2016-08-25 14:53:42Z sigurdne $
	...
	class contacts_ extends sql_builder
	{
		...
		*/
		var $map = array('contact_id'		=> array('phpgwapi.contact_central', '0'),
				 ...
				 'organizations'	=> array('phpgwapi.contact_org', '1', array('organizations_person', 'organizations_org_person', 'organizations_contact')),
				 ...
				 'people'		=> array('phpgwapi.contact_person', '1', array('people_org', 'people_org_person', 'people_contact')),
		...
		*/
		function add($owner,$fields,$access='',$cat_id='',$tid='n')
		{
			...
			{
				$criteria = $this->criteria_for_index($GLOBALS['phpgw_info']['user']['account_id'], PHPGW_CONTACTS_CATEGORIES_ALL, array('org_name'), $fields['organization']);
		...
		*/
		function add_single_extra_field($id,$owner,$field_name,$field_value)
		{
			...
			$this->other_fields->insert(array('other_contact_id' => $id, 'other_name' => $field_name, 'other_value' => $field_value));
		}
		...
		*/
		function get_email($contacts_id, $action=PHPGW_SQL_RUN_SQL)
		{
			...
			$this->criteria(array('comm_descr' => $this->search_comm_descr('work email'), 'comm_contact_id' => $contacts_id));
		...
		*/
		...
		{
			$this->request(array('comm_data'));
			$this->criteria(array('comm_descr' => $this->search_comm_descr('work phone'), 'comm_contact_id' => $contacts_id));
		...
		function comms_fixed($comm_type, $comm_description)
		{
			...
			if(!$this->hash_comms_import)
			{
				$this->hash_comms_import =
					array('email' => array('home'		=> 'home email',
											'work'		=> 'work email',
											'other'		=> 'other email'),
...
*/
		...
		*/
		...
		{
			$this->request(array('organizations', 'person_id', 'per_full_name', 'per_first_name', 'per_middle_name', 'per_last_name'));
			$this->criteria(array('org_id' => $org_id));
...
*/
		...
		*/
		...
		{
			$result = ldap_search($this->LDAPResource, $GLOBALS['phpgw_info']['server']['ldap_contact_context'], 'employeeNumber='.$id, array('dn'));
