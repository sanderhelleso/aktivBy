<?php
  ...
  *  option) any later version.                                              *
  ...
  /* $Id: class.uiserver.inc.php 13892 2015-09-14 20:07:23Z sigurdne $ */
	...
	class uiserver
	{
		...
		function edit()
		{
			...
			if ( isset($_POST['submit']) && $_POST['submit'] )
			{
				...
				if($tmp && isset($_POST['server_id']) && $_POST['server_id'])
				{
				...
				if (!$error)
				{
					//FIXME use proper escaping, but not needed as this isn't currently used
					$server_info = array(
						...
						'password'    => isset($_POST['server_password']) && $_POST['server_password'] ? $_POST['server_password'] : $server['password'],
					...
					{
						$server_info['server_id'] = $_GET['server_id'];
					}
			...
			if ( isset($_POST['submit']) && $_POST['submit'] && (!$error) && (!$errorcount))
			{
				if ( isset($_GET['server_id']) && $_GET['server_id'] )
				{
