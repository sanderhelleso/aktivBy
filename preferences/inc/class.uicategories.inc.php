<?php
	...
	* @package preferences
	* @version $Id: class.uicategories.inc.php 1153 2008-06-05 11:44:27Z sigurd $
	*/
...
//------------------- list header variable template-declarations ------------------------- 
			...
			if ( count($edata) )
			{
				...
				{
					$GLOBALS['phpgw']->template->set_var('sort_data','<th>' . lang($data) . '</th>', true);
...
// -------------------------- end header declaration --------------------------------------
			...
			{
				$GLOBALS['phpgw']->template->set_var('tr_class', $this->nextmatchs->alternate_row_class($i) );
		...
		function add()
		{
			...
			if ($extra)
			{
				...
				{
					$GLOBALS['phpgw']->template->set_var('tr_class', $this->nextmatchs->alternate_row_class($i));
					$GLOBALS['phpgw']->template->set_var('td_data','<input name="cat_data[' . $edata[$i] . ']" size="50" value="' . (isset($cat_data[$edata[$i]]) ? $cat_data[$edata[$i]] : '') . '" />');
