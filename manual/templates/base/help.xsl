<!-- $Id: help.xsl 14728 2016-02-11 22:28:46Z sigurdne $ -->
...
<xsl:template match="control_link">
	<xsl:variable name="param_url">
		<xsl:value-of select="param_url"/>
	</xsl:variable>
	<xsl:variable name="link_img">
		<xsl:value-of select="link_img"/>
	</xsl:variable>
	<xsl:variable name="img_width">
		<xsl:value-of select="img_width"/>
	</xsl:variable>
	<xsl:variable name="lang_param_title">
		<xsl:value-of select="lang_param_title"/>
	</xsl:variable>
...
<xsl:template match="listbox">
	...
	<xsl:variable name="lang_link_statustext">
		<xsl:value-of select="lang_link_statustext"/>
	</xsl:variable>
	<li>
		<a href="{$link}" target="_blank" onMouseover="window.status='{$lang_link_statustext}';return true;" onMouseout="window.status='';return true;">
