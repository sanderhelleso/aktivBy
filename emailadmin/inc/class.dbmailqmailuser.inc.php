<?php
	...
	* option) any later version.                                                *
	\***************************************************************************/
	/* $Id: class.dbmailqmailuser.inc.php 13902 2015-09-15 10:21:26Z sigurdne $ */
	...
	class dbmailqmailuser extends defaultimap {
		...
		function getUserData($_username) {
			...
			$filter		= '(&(objectclass=posixaccount)(uid='. $_username .')(qmailGID='. sprintf("%u", crc32($GLOBALS['phpgw_info']['server']['install_id'])) .'))';
		...
		function updateAccount($_hookValues) {
			...
			if($info = ldap_get_entries($ds, $sri)) {
				if(!in_array('qmailuser',$info[0]['objectclass']) && $info[0]['email']) {
					...
					$newData['qmailGID']	= sprintf("%u", crc32($GLOBALS['phpgw_info']['server']['install_id']));
