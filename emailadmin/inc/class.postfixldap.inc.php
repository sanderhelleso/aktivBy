<?php
	...
	* option) any later version.                                                *
	\***************************************************************************/
	/* $Id: class.postfixldap.inc.php 4237 2009-11-27 23:17:21Z sigurd $ */
	...
	class postfixldap extends defaultsmtp
	{
		...
		function addAccount($_hookValues)
		{
			$mailLocalAddress = $_hookValues['account_email'] ? $_hookValues['account_email'] :
				$GLOBALS['phpgw']->common->email_address($_hookValues['account_firstname'],
		...
		function updateAccount($_hookValues)
		{
			...
			if (!($sri = @ldap_search($ds, $GLOBALS['phpgw_info']['server']['ldap_context'], $filter, $attributes)))
			{
				return false;
			}
			$allValues 	= ldap_get_entries($ds, $sri);
		...
		function getUserData($_uidnumber) 
		{
			...
			if (($sri = @ldap_search($ldap,$GLOBALS['phpgw_info']['server']['ldap_context'],"(uidnumber=$_uidnumber)")))
			{
		...
		function setUserData($_uidnumber, $_mailAlternateAddress, $_mailForwardingAddress, $_deliveryMode, $_accountStatus, $_mailLocalAddress) 
		{
			...
			if (!($sri = @ldap_search($ldap,$GLOBALS['phpgw_info']['server']['ldap_context'],$filter)))
			{
				return false;
			}
			$allValues 	= ldap_get_entries($ldap, $sri);
