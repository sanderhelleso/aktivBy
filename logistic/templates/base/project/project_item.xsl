<!-- $Id: project_item.xsl 15784 2016-10-03 07:58:54Z sigurdne $ -->
<!-- item  -->
			...
			<div id="details">
				...
				<form action="{$action_url}" method="post">
					...
					<dl class="proplist-col">
						<dt>
							<label for="name">
								<xsl:value-of select="php:function('lang','Project title')" />
							</label>
						</dt>
						<dd>
							<xsl:choose>
								<xsl:when test="editable">
									...
									<div style="margin-left:0; margin-bottom: 3px;" class="help_text line">
						...
						<dt>
							<label for="project_type">
								<xsl:value-of select="php:function('lang','Project_type')" />
							</label>
						</dt>
						<dd>
							<xsl:choose>
						...
						<dt>
							<label for="copy_project_activities">
								<xsl:value-of select="php:function('lang','Copy activities from')" />
							</label>
						</dt>
						<dd>
							<xsl:choose>
								<xsl:when test="editable = '1' and activities !='1' ">
									<select id="copy_project_activities" name="copy_project_activities">
									...
									<input class="date" id="start_date" name="start_date" type="text">
										<xsl:if test="project/start_date != ''">
											<xsl:attribute name="value">
												<xsl:value-of select="php:function('date', $date_format, number(project/start_date))"/>
											</xsl:attribute>
										</xsl:if>
									</input>
									<span class="help_text line">
								...
								<xsl:when test="editable">
									...
									<input class="date" id="end_date" name="end_date" type="text">
										<xsl:if test="project/end_date != ''">
											<xsl:attribute name="value">
												<xsl:value-of select="php:function('date', $date_format, number(project/end_date))"/>
											</xsl:attribute>
										</xsl:if>
									</input>
									<span class="help_text line">
					...
					<div class="form-buttons">
						<xsl:choose>
							<xsl:when test="editable">
								...
								<a class="btn" href="{$view_projects_url}">
							...
							<xsl:otherwise>
								...
								<a class="btn" href="{$view_projects_url_2}">
